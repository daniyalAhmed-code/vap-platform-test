...
exports.handler = async (req, res) => {
      ...
      const schema = Joi.object().keys({
        ClientName: Joi.string().regex(/^(?=.{3,50}$)[a-zA-Z]+(?:['_.\s][a-z]+)*$/).required().messages({
          'string.base': `"Resource name" should be a type of 'text'`,
          'string.empty': `"Resource name" cannot be an empty field`,
          'string.pattern.base': "Resource name cannot have space in between",
          'string.min': `"Resource name" should have a minimum length of 3`,
