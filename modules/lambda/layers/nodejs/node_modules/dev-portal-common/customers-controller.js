...
async function createExistingUsersApiKey (userId,identityId,enabled) {
  console.log(`Creating API Key for identity ID [${userId}]`)
  let user = await getAccountDetails(userId)
  let stages = [common.stages.alpha,common.stages.beta,common.stages.production]
  let user_stage = []
  let api_details = {}
  let user_details = {}
  
  user_details['KeyRotaionEnabled'] = user.KeyRotaionEnabled
  user_details['KeyDuration'] = user.KeyDuration
  user_details['CallbackAuthType'] = user.CallbackAuthType
  user_details['CallBackAuthARN'] = user.CallBackAuthARN
  user_details['CallBackUrl'] = user.CallBackUrl

for (let stage of stages) {
  
  if(stage == common.stages.alpha)
      api_details[stage] ={"id" : user.ApiKeyId, ...user_details}
  
  else{
      let apiKey =await apigateway
    .createApiKey({
      description: `Dev Portal API Key for account with Identity Pool user ${identityId} / User Pool user ${userId}`,
      enabled,
      generateDistinctId: true,
    ...
    .promise()
    api_details[stage] ={"id":apiKey.id, ...user_details}
  }
  }
  user_stage.push(api_details)
  let updateCustomer = await updateExistingCustomerApiKeyId (userId,user_stage)
    return updateCustomer
  }

async function updateExistingCustomerApiKeyId (cognitoIdentityId, data) {
    ...
    let updateUsers = dynamoDb.update(dynamoDbParams).promise()
    return updateUsers
  }  
