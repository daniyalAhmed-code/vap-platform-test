...
const createResourceDetails = async (
  ...
  if (ClientType == "Mno")
    registeredGroupName = `${process.env.MnoAdminGroupName}`
  else 
    registeredGroupName = `${process.env.ThirdPartyAdminGroupName}`
    ...
    try {
      ...
      await dynamoDb
        ...
        const updateUserAttribues = await cognitoIdp
          ...
          .promise()
          let resp = await cognitoIdp.adminAddUserToGroup({
            GroupName: registeredGroupName,
            UserPoolId: UserPoolId,
            Username: targetUser.Username
          })
...
const updateProfileImageLocation = async (
  user_id,
  data
