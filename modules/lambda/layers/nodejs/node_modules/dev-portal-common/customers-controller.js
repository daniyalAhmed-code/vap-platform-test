 ...
 */
 async function __createApiKey ({ identityId, userId, stage, enabled }) {
...
//  await promisify2(updateCustomerApiKeyId)(identityId,stage, apiKey.id)
 ...
 */
 const ensureApiKeyForCustomer = async ({ userId, identityId, stage }) => {
  ...
  try {
    ...
    console.log(existingApiKeys.items.length)
    const existingApiKey = existingApiKeys.items[0]
    
    if (existingApiKey && existingApiKey.enabled !== isRegistered &&  existingApiKeys.items.length ==3) {
      console.log(
    ...
    } else if (existingApiKey === undefined || existingApiKeys.items.length < 3) {
 ...
 */
 function updateCustomerApiKeyId (cognitoIdentityId,Apis,error, success) {
  ...
  const dynamoDbParams = {
    ...
    UpdateExpression: `set ApiKeyId.stage = :x`,
    ExpressionAttributeValues: {
      ':x': Apis,
    }
  }
...
const saveInvitePreLoginAccount = async ({
  ...
  inviterEmailAddress
   ...
   for (let [index,api] of Apis.stage.entries()){
  var params = {
      Description: "User CallbackAuth type", 
      Name :  "vap/production/user-callback-auth/"+username+"/"+index+1, 
     };
    
   if(api.type == "apiKey")  {
    params['SecretString'] = JSON.stringify({ "apiKey": api.callBackAuth });
  }
  else if (api.type == "basicAuth"){
    params['SecretString'] = JSON.stringify({ "username": api.callBackAuth.username,  "password": api.callBackAuth.password});  
  }
  else if (api.type == "privateCertificate"){
    params['SecretString'] = JSON.stringify({ "PrivateCertificate": api.callBackAuth });
 } 
  delete api.callBackAuth
  api['CallBackAuthARN'] = await secretsmanager.createSecret(params).promise()
  }

  await __savePreLoginAccount({
    UserId: userId,
    Username: username,
    EmailAddress: emailAddress,
...
const listAccountsByRegistrationStatus = async registrationStatus => {
  ...
  try {
    const filterParams = {
      FilterExpression: 'RegistrationStatus = :status and ClientType = :su',
      ExpressionAttributeValues: {
...
let updateAccountDetails = async (
  ...
  for (let stage of user_details ){
    console.log(stage)
    console.log(typeof stage)

    let secretManagerParams = {
    SecretId: stage.CallBackAuthARN
  }

  if (stage.CallBackAuth !="NONE")
  {  
    if(stage.CallbackAuthType == "apiKey")  {
      secretManagerParams['SecretString'] = JSON.stringify({ "apiKey": stage.CallBackAuth });
    }
    else if (stage.CallbackAuthType == "basicAuth"){
      secretManagerParams['SecretString'] = JSON.stringify({ "username": stage.CallBackAuth.username,  "password": stage.CallBackAuth.password});  
    }
    else if (stage.CallbackAuthType == "privateCertificate"){
      secretManagerParams['SecretString'] = JSON.stringify({ "PrivateCertificate": stage.CallBackAuth });
    } 
...
async function renewApiKey ( identityId, userId,stage, enabled ) {
  console.log(`Creating API Key for identity ID [${identityId}]`)
  let updatedApiKey
  try {
...
const createResourceDetails = async (
  ClientId,
  ClientType,
  UserPoolId,
  EmailAddress,
  ...
  const createUserResponse = await cognitoIdp
    .adminCreateUser({
      ...
      DesiredDeliveryMediums:["EMAIL"],
      TemporaryPassword: "Pa$$Word123123",
       UserAttributes: [ 
        { 
           "Name": "phone_number",
           "Value": PhoneNumber
    ...
    try {
      await dynamoDb.put({
        TableName: `${process.env.MnoThirdPartyResourceTableName}`,
        Item:{
          "Id":uuid,
          "ClientName":ClientId,
      ...
      await dynamoDb
        .put({
          ...
          Item: {
            ...
            "ClientType":ClientType,
            "UserPoolId":createUserResponse.User.Username,
            "ClientId":ClientId,
            "EmailAddress":EmailAddress,
            "FirstName":FirstName,
            "LastName":LastName,
...
const createPermissionToAccessApis = async (ClientType,ClientId,ApiId) =>{
  let tableName = `${process.env.ApiPermissionTableName}`
  try{
    let resource_permission = await dynamoDb.put({
      TableName:tableName,
      Item:{
...
const getAllowedApisForResource = async (ClientId) =>{
  let tableName = `${process.env.ApiPermissionTableName}`
  try{
    let details = await dynamoDb.get({
      TableName:tableName,
      Key:{
        "Id": ClientId
      }
...
const getAllApisForResource = async (ClientId) =>{
  let tableName = `${process.env.ApiPermissionTableName}`
  try{
    let details = await dynamoDb.scan({
      TableName:tableName
    }).promise()
  return details
  }
...
const deleteAllowedApisForResource = async (ClientId) =>{
  let tableName = `${process.env.ApiPermissionTableName}`
  try{
...
const updateAllowedApisForResource = async (ClientId,ApiId) =>{
  let tableName = `${process.env.ApiPermissionTableName}`
  try{
    const dynamoDbParams = {
      TableName: tableName,
      ...
      UpdateExpression: 'set #a = :x',
      ExpressionAttributeNames: { '#a': 'ApiId' },
      ExpressionAttributeValues: {
        ':x': ApiId,
      }
    }
...
const getAllResourceDetails = async (ClientType) =>{
  console.log(ClientType)
  let params = {
    FilterExpression : 'ClientType = :rt',
  ExpressionAttributeValues : {':rt' : ClientType},
    TableName: `${process.env.MnoThirdPartyResourceTableName}`,
    
   };
  let resource_details = await dynamoDb.scan(params).promise()
...
const getResourceById = async (ResourceId) =>{
  ...
  let resource_details = await dynamoDb.query(params).promise()
  console.log(resource_details)
  return resource_details
}

const getResourceUsers = async (ResourceType) =>{
  console.log(ResourceType)
  let params = {
    FilterExpression : 'ClientType = :rt',
    ExpressionAttributeValues : {':rt' : ResourceType},
    TableName: `${process.env.CustomersTableName}`
   };
  let resource_details = await dynamoDb.scan(params).promise()
  console.log(resource_details)
  return resource_details
...
const getResourceUser = async (ResourceId,ResourceType) =>{
  console.log(ResourceType)
  let params = {
    FilterExpression : 'ClientType = :rt and Id :id',
    ExpressionAttributeValues : {':rt' : ResourceType, ":id" : ResourceId},
    TableName: `${process.env.CustomersTableName}`
   };
  let resource_details = await dynamoDb.scan(params).promise()
  console.log(resource_details)
...
const createResource = async (
 ...
 const updateUserAttribues = await cognitoIdp
  ...
  if(ClientType = "Mno")
    Client_details['MnoLocation'] = MnoLocation

   console.log("is after updateUserAttributes")
   try {
     console.log(createUserResponse.User)
     console.log("is after dynamodb")
     await dynamoDb
...
const listAllUsers = async () =>{
  ...
  let getAllUsers = await dynamoDb.scan(params).promise()

  return getAllUsers
}
async function createExistingUsersApiKey ({ identityId, userId, apiKeyId, enabled }) {
  console.log(`Creating API Key for identity ID [${identityId}]`)
  let stages = [common.stages.alpha,common.stages.beta,common.stages.prod]
  let apiKey = {}
  let data = {}
  for (let stage of stages) {
  if(stage == common.stages.alpha)
    {apiKey['id'] = apiKeyId
    }
  else
{  try {
  ...
  console.log(`Created API key with ID [${apiKey.id}]`)
  data[stage] = apiKey.id
  
}
 await promisify2(updateExistingCustomerApiKeyId)(identityId,data)
  
  return apiKey
}

function updateExistingCustomerApiKeyId (cognitoIdentityId, data,error, success) {
  // update customer record with marketplace customer code
  console.log(data)
  ...
  const dynamoDbParams = {
    ...
    UpdateExpression: `set ApiKeyId = :x`,
    ExpressionAttributeValues: {
      ':x': data,
    }
  }
...
exports = module.exports = {
  ...
  createPermissionToAccessApis,
  getAllowedApisForResource,
  deleteAllowedApisForResource,
  updateAllowedApisForResource,
  ...
  createResource,
  deleteResource,
  getAllApisForResource,
  listAllUsers,
  createExistingUsersApiKey,
  updateCustomerApiKeyId
}
